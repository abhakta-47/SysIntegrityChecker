...
const CONFIG_STORAGE_KEY = 'PROCTORING_CONFIG';
...
function loadConfigFromStorage() {
    try {
        ...
        if (stored) {
            ...
            addLog('Configuration loaded from localStorage', 'info', 'localStorage.getItem');
            return parsed;
        }
    } catch (error) {
        console.error('Failed to load config from localStorage:', error);
        addLog('Failed to load saved config, using defaults', 'warning', 'localStorage.getItem');
    }
    
    // First time load - save default config
    saveConfigToStorage(DEFAULT_PROCTORING_CONFIG);
    addLog('First load: Default configuration saved', 'info', 'localStorage.setItem');
...
function saveConfigToStorage(config) {
    try {
        ...
        addLog('Failed to save configuration', 'error', 'localStorage.setItem');
        return false;
    }
}

// Load configuration (from localStorage or default)
let PROCTORING_CONFIG = loadConfigFromStorage();
...
function openConfigModal() {
    ...
    textarea.value = JSON.stringify(PROCTORING_CONFIG, null, 2);
    ...
    addLog('Configuration editor opened', 'info', 'UI');
}

function closeConfigModal() {
    const modal = document.getElementById('configModal');
    modal.classList.remove('active');
    addLog('Configuration editor closed', 'info', 'UI');
}
...
function saveConfig() {
    ...
    try {
        ...
        if (saveConfigToStorage(newConfig)) {
            addLog('Configuration saved successfully', 'success', 'localStorage.setItem');
            addLog('Reloading page to apply changes...', 'info', 'window.location.reload');
        ...
        addLog(`Configuration save failed: ${error.message}`, 'error', 'JSON.parse');
    }
}
...
function resetToDefault() {
    if (confirm('Are you sure you want to reset to default configuration? This will reload the page.')) {
        saveConfigToStorage(DEFAULT_PROCTORING_CONFIG);
        addLog('Configuration reset to defaults', 'info', 'localStorage.setItem');
        addLog('Reloading page...', 'info', 'window.location.reload');
